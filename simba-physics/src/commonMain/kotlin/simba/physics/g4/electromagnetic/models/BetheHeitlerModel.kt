...
class BetheHeitlerModel(
    parametrisation: Parametrisation? = null,
    val units: UnitsSpace = SI_UNITS,
    val consts: ConstsSpace = SI_CONSTS
) : HEPDiscreteModel {
    val parametrisation = parametrisation ?: defaultParametrisation(units)

    companion object {
        fun defaultParametrisation(unitsSpace: UnitsSpace) = with(unitsSpace) {
            Parametrisation(
...
//            const G4Material* aMaterial = couple->GetMaterial()
        ...
        val Z = element.Z
        var epsil: Double
        val epsil0 = consts.electron_mass_c2 / gammaEnergy
        if (epsil0 > 1.0) {
            return emptyList()
        }
...
//            SelectRandomAtom(aMaterial, theGamma, GammaEnergy)
        ...
        val thetaEle = u * consts.electron_mass_c2 / ElectTotEnergy
        val sinte = sin(thetaEle)
        val coste = cos(thetaEle)

        val thetaPos = u * consts.electron_mass_c2 / PositTotEnergy
        val sintp = sin(thetaPos)
        val costp = cos(thetaPos)

        val phi = consts.twopi * rnd.nextDouble()
        val sinp = sin(phi)
        val cosp = cos(phi)
        ...
        val ElectKineEnergy = max(0.0, ElectTotEnergy - consts.electron_mass_c2)
        ...
        val PositKineEnergy = max(0.0, PositTotEnergy - consts.electron_mass_c2)
